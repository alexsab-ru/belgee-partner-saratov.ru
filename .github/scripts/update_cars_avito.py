...
for car in cars_element:
    ...
    duplicates = duplicate_car(car, 1, "в наличии", len(all_duplicates))
    all_duplicates.extend(duplicates)  # Добавляем дубликаты в отдельный список
    
    duplicates = duplicate_car(car, 7, "в пути", len(all_duplicates))
    all_duplicates.extend(duplicates)  # Добавляем дубликаты в отдельный список
    
    duplicates = duplicate_car(car, 1, "на заказ", len(all_duplicates))
    all_duplicates.extend(duplicates)  # Добавляем дубликаты в отдельный список
